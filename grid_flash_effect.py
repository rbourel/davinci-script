...
# Pas d'import necessaire : "resolve", "fusion" et "bmd" sont deja definis dans le
# contexte de la Console Resolve, du menu Workspace > Scripts, ou des render scripts.
...
FLASH_START_BRIGHTNESS = 1.0     # 1.0 = neutre (image normale)
FLASH_END_BRIGHTNESS = 0.0       # 0.0 = noir
# ===============================================================================================
...
def apply_crop_and_flash(item, r, c, grid_rows, grid_cols, timeline_width, timeline_height, add_flash):
    left = c / grid_cols
...
def generate_grid(grid_rows, grid_cols, stagger_frames, add_flash):
    ...
    # balayage haut -> bas, puis gauche -> droite
    ...
    for i, (r, c) in enumerate(tiles):
        ...
        apply_crop_and_flash(item, r, c, grid_rows, grid_cols, timeline_width, timeline_height, add_flash)
        placed += 1
...
def on_generate(ev):
    ...
    win.Hide()
    try:
        generate_grid(rows, cols, stagger, flash)
